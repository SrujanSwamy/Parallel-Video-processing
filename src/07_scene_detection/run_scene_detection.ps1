...
Write-Host "        SCENE DETECTION - PERFORMANCE COMPARISON" -ForegroundColor Cyan
Write-Host "================================================================" -ForegroundColor Cyan
Write-Host ""

# Create output directory if it doesn't exist
$outputDir = "outputs\07_scene_detection"
...
& .\compile.ps1 -Program scene_detection_sequential

Write-Host "`n>>> Running Sequential version..." -ForegroundColor Green
$output = & .\build\scene_detection_sequential.exe $VideoFile 2>&1 | Out-String
Write-Host $output
...
& .\compile.ps1 -Program scene_detection_pthread

Write-Host "`n>>> Running Pthread version with $NumThreads threads..." -ForegroundColor Green
$output = & .\build\scene_detection_pthread.exe $VideoFile $NumThreads 2>&1 | Out-String
Write-Host $output
...
& .\compile.ps1 -Program scene_detection_openmp

Write-Host "`n>>> Running OpenMP version with $NumThreads threads..." -ForegroundColor Green
$output = & .\build\scene_detection_openmp.exe $VideoFile $NumThreads 2>&1 | Out-String
Write-Host $output
...
Write-Host "  - $outputDir\scene_detection_sequential.txt" -ForegroundColor White
Write-Host "  - $outputDir\scene_detection_pthread.txt" -ForegroundColor White
Write-Host "  - $outputDir\scene_detection_openmp.txt" -ForegroundColor White
Write-Host ""
