...
Write-Host "      CONTRAST ENHANCEMENT - PERFORMANCE COMPARISON" -ForegroundColor Cyan
Write-Host "================================================================" -ForegroundColor Cyan
Write-Host ""

# Create output directory if it doesn't exist
$outputDir = "outputs\11_contrast_enhancement"
...
& .\compile.ps1 -Program contrast_enhancement_sequential

Write-Host "`n>>> Running Sequential version..." -ForegroundColor Green
$output = & .\build\contrast_enhancement_sequential.exe $VideoFile 2>&1 | Out-String
Write-Host $output
...
& .\compile.ps1 -Program contrast_enhancement_pthread

Write-Host "`n>>> Running Pthread version with $NumThreads threads..." -ForegroundColor Green
$output = & .\build\contrast_enhancement_pthread.exe $VideoFile $NumThreads 2>&1 | Out-String
Write-Host $output
...
& .\compile.ps1 -Program contrast_enhancement_openmp

Write-Host "`n>>> Running OpenMP version with $NumThreads threads..." -ForegroundColor Green
$output = & .\build\contrast_enhancement_openmp.exe $VideoFile $NumThreads 2>&1 | Out-String
Write-Host $output
...
Write-Host "  - $outputDir\contrast_enhancement_sequential.avi" -ForegroundColor White
Write-Host "  - $outputDir\contrast_enhancement_pthread.avi" -ForegroundColor White
Write-Host "  - $outputDir\contrast_enhancement_openmp.avi" -ForegroundColor White
Write-Host ""
