...
Write-Host "      BRIGHTNESS/CONTRAST - PERFORMANCE COMPARISON" -ForegroundColor Cyan
Write-Host "================================================================" -ForegroundColor Cyan
Write-Host ""

# Create output directory if it doesn't exist
$outputDir = "outputs\09_brightness_contrast"
...
& .\compile.ps1 -Program brightness_contrast_sequential

Write-Host "`n>>> Running Sequential version..." -ForegroundColor Green
$output = & .\build\brightness_contrast_sequential.exe $VideoFile 2>&1 | Out-String
Write-Host $output
...
& .\compile.ps1 -Program brightness_contrast_pthread

Write-Host "`n>>> Running Pthread version with $NumThreads threads..." -ForegroundColor Green
$output = & .\build\brightness_contrast_pthread.exe $VideoFile $NumThreads 2>&1 | Out-String
Write-Host $output
...
& .\compile.ps1 -Program brightness_contrast_openmp

Write-Host "`n>>> Running OpenMP version with $NumThreads threads..." -ForegroundColor Green
$output = & .\build\brightness_contrast_openmp.exe $VideoFile $NumThreads 2>&1 | Out-String
Write-Host $output
...
Write-Host "  - $outputDir\brightness_contrast_sequential.avi" -ForegroundColor White
Write-Host "  - $outputDir\brightness_contrast_pthread.avi" -ForegroundColor White
Write-Host "  - $outputDir\brightness_contrast_openmp.avi" -ForegroundColor White
Write-Host ""
