...
Write-Host "         WHITE BALANCE - PERFORMANCE COMPARISON" -ForegroundColor Cyan
Write-Host "================================================================" -ForegroundColor Cyan
Write-Host ""

# Create output directory if it doesn't exist
$outputDir = "outputs\04_white_balance"
...
& .\compile.ps1 -Program white_balance_sequential

Write-Host "`n>>> Running Sequential version..." -ForegroundColor Green
$output = & .\build\white_balance_sequential.exe $VideoFile 2>&1 | Out-String
Write-Host $output
...
& .\compile.ps1 -Program white_balance_pthread

Write-Host "`n>>> Running Pthread version with $NumThreads threads..." -ForegroundColor Green
$output = & .\build\white_balance_pthread.exe $VideoFile $NumThreads 2>&1 | Out-String
Write-Host $output
...
& .\compile.ps1 -Program white_balance_openmp

Write-Host "`n>>> Running OpenMP version with $NumThreads threads..." -ForegroundColor Green
$output = & .\build\white_balance_openmp.exe $VideoFile $NumThreads 2>&1 | Out-String
Write-Host $output
...
Write-Host "  - $outputDir\white_balance_sequential.avi" -ForegroundColor White
Write-Host "  - $outputDir\white_balance_pthread.avi" -ForegroundColor White
Write-Host "  - $outputDir\white_balance_openmp.avi" -ForegroundColor White
Write-Host ""
