...
Write-Host "      BACKGROUND SUBTRACTION - PERFORMANCE COMPARISON" -ForegroundColor Cyan
Write-Host "================================================================" -ForegroundColor Cyan
Write-Host ""

# Create output directory if it doesn't exist
$outputDir = "outputs\08_background_subtraction"
...
& .\compile.ps1 -Program background_subtraction_sequential

Write-Host "`n>>> Running Sequential version..." -ForegroundColor Green
$output = & .\build\background_subtraction_sequential.exe $VideoFile 2>&1 | Out-String
Write-Host $output
...
& .\compile.ps1 -Program background_subtraction_pthread

Write-Host "`n>>> Running Pthread version with $NumThreads threads..." -ForegroundColor Green
$output = & .\build\background_subtraction_pthread.exe $VideoFile $NumThreads 2>&1 | Out-String
Write-Host $output
...
& .\compile.ps1 -Program background_subtraction_openmp

Write-Host "`n>>> Running OpenMP version with $NumThreads threads..." -ForegroundColor Green
$output = & .\build\background_subtraction_openmp.exe $VideoFile $NumThreads 2>&1 | Out-String
Write-Host $output
...
Write-Host "  - $outputDir\background_subtraction_sequential.avi" -ForegroundColor White
Write-Host "  - $outputDir\background_subtraction_pthread.avi" -ForegroundColor White
Write-Host "  - $outputDir\background_subtraction_openmp.avi" -ForegroundColor White
Write-Host ""
