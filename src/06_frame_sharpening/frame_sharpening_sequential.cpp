...
void applySharpen(const Mat &frame, Mat &output) {
	Mat blurred;
...
int main(int argc, const char** argv) {
	...
	if (argc < 2) {
		...
		printf("Example: %s input_videos/sample_video.mp4 outputs/06_frame_sharpening/output_sequential.avi\n", argv[0]);
		return 0;
	}
	
	// Set output path
	string outputPath = (argc >= 3) ? argv[2] : "outputs/06_frame_sharpening/frame_sharpening_sequential.avi";
	...
	printf("Processing video (Sequential Frame Sharpening)...\n");
	...
	while (true) {
		...
		// Apply sharpening
		applySharpen(frame, sharpened);
		
		// Write output
		if (OUTPUT_VIDEO) {
			outputVideo << sharpened;
		}
	...
	printf("\n\n");
	printf("========================================\n");
	printf("Frame Sharpening - Sequential\n");
	printf("========================================\n");
