...
Write-Host "        FRAME SHARPENING - PERFORMANCE COMPARISON" -ForegroundColor Cyan
Write-Host "================================================================" -ForegroundColor Cyan
Write-Host ""

# Create output directory if it doesn't exist
$outputDir = "outputs\06_frame_sharpening"
...
& .\compile.ps1 -Program frame_sharpening_sequential

Write-Host "`n>>> Running Sequential version..." -ForegroundColor Green
$output = & .\build\frame_sharpening_sequential.exe $VideoFile 2>&1 | Out-String
Write-Host $output
...
& .\compile.ps1 -Program frame_sharpening_pthread

Write-Host "`n>>> Running Pthread version with $NumThreads threads..." -ForegroundColor Green
$output = & .\build\frame_sharpening_pthread.exe $VideoFile $NumThreads 2>&1 | Out-String
Write-Host $output
...
# 3. OPENMP VERSION
# ============================================================================
Write-Host ">>> Building OpenMP version..." -ForegroundColor Yellow
& .\compile.ps1 -Program frame_sharpening_openmp

Write-Host "`n>>> Running OpenMP version with $NumThreads threads..." -ForegroundColor Green
$output = & .\build\frame_sharpening_openmp.exe $VideoFile $NumThreads 2>&1 | Out-String
Write-Host $output
...
Write-Host "  - $outputDir\frame_sharpening_sequential.avi" -ForegroundColor White
Write-Host "  - $outputDir\frame_sharpening_pthread.avi" -ForegroundColor White
Write-Host "  - $outputDir\frame_sharpening_openmp.avi" -ForegroundColor White
Write-Host ""
