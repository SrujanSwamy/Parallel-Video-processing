...
Write-Host "      MOTION BLUR REDUCTION - PERFORMANCE COMPARISON" -ForegroundColor Cyan
Write-Host "================================================================" -ForegroundColor Cyan
Write-Host ""

# Create output directory if it doesn't exist
$outputDir = "outputs\10_motion_blur_reduction"
...
& .\compile.ps1 -Program motion_blur_reduction_sequential

Write-Host "`n>>> Running Sequential version..." -ForegroundColor Green
$output = & .\build\motion_blur_reduction_sequential.exe $VideoFile 2>&1 | Out-String
Write-Host $output
...
& .\compile.ps1 -Program motion_blur_reduction_pthread

Write-Host "`n>>> Running Pthread version with $NumThreads threads..." -ForegroundColor Green
$output = & .\build\motion_blur_reduction_pthread.exe $VideoFile $NumThreads 2>&1 | Out-String
Write-Host $output
...
& .\compile.ps1 -Program motion_blur_reduction_openmp

Write-Host "`n>>> Running OpenMP version with $NumThreads threads..." -ForegroundColor Green
$output = & .\build\motion_blur_reduction_openmp.exe $VideoFile $NumThreads 2>&1 | Out-String
Write-Host $output
...
Write-Host "  - $outputDir\motion_blur_reduction_sequential.avi" -ForegroundColor White
Write-Host "  - $outputDir\motion_blur_reduction_pthread.avi" -ForegroundColor White
Write-Host "  - $outputDir\motion_blur_reduction_openmp.avi" -ForegroundColor White
Write-Host ""
