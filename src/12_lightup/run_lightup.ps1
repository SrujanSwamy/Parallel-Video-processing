...
Write-Host "             LIGHTUP - PERFORMANCE COMPARISON" -ForegroundColor Cyan
Write-Host "================================================================" -ForegroundColor Cyan
Write-Host ""

# Create output directory if it doesn't exist
$outputDir = "outputs\12_lightup"
...
& .\compile.ps1 -Program lightup_sequential

Write-Host "`n>>> Running Sequential version..." -ForegroundColor Green
$output = & .\build\lightup_sequential.exe $VideoFile 2>&1 | Out-String
Write-Host $output
...
& .\compile.ps1 -Program lightup_pthread

Write-Host "`n>>> Running Pthread version with $NumThreads threads..." -ForegroundColor Green
$output = & .\build\lightup_pthread.exe $VideoFile $NumThreads 2>&1 | Out-String
Write-Host $output
...
& .\compile.ps1 -Program lightup_openmp

Write-Host "`n>>> Running OpenMP version with $NumThreads threads..." -ForegroundColor Green
$output = & .\build\lightup_openmp.exe $VideoFile $NumThreads 2>&1 | Out-String
Write-Host $output
...
Write-Host "  - $outputDir\lightup_sequential.avi" -ForegroundColor White
Write-Host "  - $outputDir\lightup_pthread.avi" -ForegroundColor White
Write-Host "  - $outputDir\lightup_openmp.avi" -ForegroundColor White
Write-Host ""
